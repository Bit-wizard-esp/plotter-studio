...
├── tests/
│   ├── test_pipeline.py          # ядро: режимы, проходы, границы стола/изображения
│   └── test_ui_smoke.py          # UI offscreen: вкладки, режимы, генерация, темы
└── tools/
    ├── gen_icons.py               # генерация иконок темы
    └── shots.py                   # скриншоты окна (для документации)
```

## Тесты

```bat
python tests\test_pipeline.py   :: ядро (не нужен дисплей)
python tests\test_ui_smoke.py   :: UI (offscreen-режим Qt)
```

## Если фото не открывается

Загрузчик устойчив к типовым проблемам:

- **кириллица в имени/пути файла** — работает (файл читается байтами, а не через
  `imread`, который на Windows с кириллическими путями падает);
- **CMYK / 16-бит JPEG** (частые у стоковых фото) — читаются через Pillow;
- **расширение не соответствует содержимому** (PNG под `.jpg`) — читается;
- если всё же не открылось, в окне ошибки будет диагностика: реальный формат
  по сигнатуре, размер файла и подсказка (HEIC от iPhone, AVIF, «браузер сохранил
  страницу, а не картинку» и т. п.).

Для AVIF-картинок доустановите `pip install pillow-avif-plugin`.
